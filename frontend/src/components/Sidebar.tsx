...
import { FaBox, FaFileInvoiceDollar, FaTruck, FaCircle } from "react-icons/fa";
...
export const Sidebar = () => (
  <aside className="w-72 border-r border-white/10 flex flex-col p-8 bg-[#07080d]">
    ...
    <nav className="flex-1 space-y-2">
      <MenuLink to="/" icon={<FaBox />} label="Place Order" />
      <MenuLink
        to="/invoices"
        icon={<FaFileInvoiceDollar />}
        label="Generate Invoice"
      />
      <MenuLink to="/shipments" icon={<FaTruck />} label="Prepare Shipment" />
    </nav>
